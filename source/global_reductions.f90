...
!-----------------------------------------------------------------------
   ...
   !call timer_stop(timer_mpi)
      ...
      global_sum_dbl = global_sum_dbl_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
!
...
! !INTERFACE:
 ...
 function global_sum_dbl_tiny(X, dist, field_loc, MASK)
...
! !OUTPUT PARAMETERS:
   ...
   real (r8) :: &
      global_sum_dbl_tiny       ! resulting global sum
...
!      global_block_sum     ! sum of all blocks
   ...
   integer (int_kind) :: &
      i,j,n,             &! local counters
      ib,ie,jb,je,       &! beg,end of physical domain
      local_id,          &! local_id of root_block 
      k,iblock,jblock,   &! tiny blocks
      bid,               &! block location
      ierr                ! MPI error flag

   type (block) :: &
      root_block        ,&! holds local block information
      this_block          ! holds local block information
...
!-----------------------------------------------------------------------
   ...
   if (ltripole_grid .and. (field_loc == field_loc_Nface .or. &
      ...
      !*** must remove redundant points from sum
      local_id = 0
      do k=1,dist%local_block_num,nx_bkfactor*ny_bkfactor
        local_id = local_id+1
        this_block = get_block(dist%local_block_ids(k),k) ! tiny-block
        root_block = get_block(this_block%block_id,local_id) ! root macro-block
        if (root_block%jblock == nblocks_y) then
          !$OMP PARALLEL DO PRIVATE(jblock,iblock,bid,n,this_block,&
          !$OMP                     ib,ie,jb,je,i,j)&
          ...
          enddo ! jblock
          !$OMP END PARALLEL DO
        else ! (root_block%jblock == nblocks_y)
          !$OMP PARALLEL DO PRIVATE(jblock,iblock,bid,n,this_block,&
          !$OMP                     ib,ie,jb,je,i,j)&
          !$OMP             REDUCTION(+:local_sum)
          do jblock=1,ny_bkfactor
          ...
          enddo ! jblock
          !$OMP END PARALLEL DO
        endif ! (root_block%jblock == nblocks_y)
      end do ! k - macro block
   else ! regular global sum
      !$OMP PARALLEL DO PRIVATE(bid,n,ib,ie,jb,je,i,j)&
      !$OMP             REDUCTION(+:local_sum)
      do n=1,dist%local_block_num
            bid = n
            call get_block_parameter(dist%local_block_ids(n),ib=ib,ie=ie,jb=jb,je=je)
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_dbl_tiny, 1, &
                            mpi_dbl, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_dbl_tiny = c0
      endif
   else
      global_sum_dbl_tiny = local_sum
   endif
   !call timer_stop(timer_mpi)

 end function global_sum_dbl_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_sum_real = global_sum_real_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc
...
!***********************************************************************
 ...
 function global_sum_real_tiny(X, dist, field_loc, MASK)
...
!-----------------------------------------------------------------------
   ...
   real (r4) :: &
      global_sum_real_tiny   ! resulting global sum
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_real_tiny, 1, &
                            mpi_real, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_real_tiny = c0
      endif
   else
      global_sum_real_tiny = local_sum
   endif
 end function global_sum_real_tiny
...
!               write(6,*) 'global_sum_int:ib-je:',my_task,ib,ie,jb,je,dist%local_block_ids(n)
            if (present(MASK)) then
               do j=jb,je
               do i=ib,ie
                  local_sum = local_sum + X(i,j,bid)*MASK(i,j,bid)
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
!         if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'global_sum_int:locsum:',my_task,local_sum
      ...
      global_sum_int = global_sum_int_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc
...
!***********************************************************************
 ...
 function global_sum_int_tiny(X, dist, field_loc, MASK)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      global_sum_int_tiny   ! resulting global sum
...
!-----------------------------------------------------------------------
   ...
   if (ltripole_grid .and. (field_loc == field_loc_Nface .or. &
      ...
      do n=1,dist%local_block_num
            ...
            this_block = get_block(dist%local_block_ids(n),n) ! tiny-block
            root_block = get_block(this_block%block_id,1) ! root macro-block
!               write(6,*) 'global_sum_int_tiny:ib-je:',my_task,ib,ie,jb,je,this_block%block_id,sum(X(:,:,1:8)),sum(X(3:92,3:27,1:8))
            endif
            if (present(MASK)) then
               do j=jb,je
               do i=ib,ie
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
!         if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'global_sum_int_tiny:locsum:',my_task,local_sum
         call MPI_ALLREDUCE(local_sum, global_sum_int_tiny, 1, &
                       mpi_integer, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_int_tiny = 0
      endif
   else
      global_sum_int_tiny = local_sum
   endif

 end function global_sum_int_tiny

!***********************************************************************

 function global_sum_1d_dbl(array, dist)
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(size(array,dim=1)) :: &
      global_sum_1d_dbl   ! resulting global sum
...
!-----------------------------------------------------------------------
   ...
   numFields = size(array,dim=1)

   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(array, global_sum_1d_dbl, numFields, &
                            mpi_dbl, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_1d_dbl = c0
      endif
   else
      global_sum_1d_dbl = array
   endif
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_sum_prod_dbl = global_sum_prod_dbl_tiny(X, Y, dist, field_loc, MASK)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
!
!  use this code for sums that are more reproducible - performance
...
! !INTERFACE:
 ...
 function global_sum_prod_dbl_tiny (X,Y,dist,field_loc, MASK)
...
! !OUTPUT PARAMETERS:
   ...
   real (r8) :: &
     global_sum_prod_dbl_tiny ! resulting global sum of X*Y
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_prod_dbl_tiny, 1, &
                      mpi_dbl, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_prod_dbl_tiny = 0
      endif
   else
      global_sum_prod_dbl_tiny = local_sum
   endif

 end function global_sum_prod_dbl_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_sum_prod_real = global_sum_prod_real_tiny(X, Y, dist, field_loc, MASK)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
!
!  use this code for sums that are more reproducible - performance
...
!***********************************************************************
 ...
 function global_sum_prod_real_tiny (X, Y, dist, field_loc, MASK)
...
!-----------------------------------------------------------------------
   ...
   real (r4) :: &
     global_sum_prod_real_tiny ! resulting global sum of X*Y
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_prod_tmp, 1, &
                      mpi_real, MPI_SUM, dist%communicator, ierr)
         global_sum_prod_real_tiny = global_sum_prod_tmp
      else
         global_sum_prod_real_tiny = 0
      endif
   else
      global_sum_prod_real_tiny = local_sum
   endif

 end function global_sum_prod_real_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_sum_prod_int = global_sum_prod_int_tiny(X, Y, dist, field_loc, MASK)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
!
!  use this code for sums that are more reproducible - performance
...
!***********************************************************************
 ...
 function global_sum_prod_int_tiny (X, Y, dist, field_loc, MASK)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
     global_sum_prod_int_tiny ! resulting global sum of X*Y
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_prod_int_tiny, 1, &
                      mpi_integer, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_prod_int_tiny = 0
      endif
   else
      global_sum_prod_int_tiny = local_sum
   endif

 end function global_sum_prod_int_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_count_dbl = global_count_dbl_tiny(MASK, dist, field_loc)
   endif  ! lmic_proc
!-----------------------------------------------------------------------

 end function global_count_dbl

 function global_count_dbl_tiny (MASK, dist, field_loc)
...
!OUTPUT PARAMETERS:
   ...
   integer (int_kind) :: &
      global_count_dbl_tiny      ! resulting global count
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_count, global_count_dbl_tiny, 1, &
                         MPI_INTEGER, MPI_SUM, dist%communicator, ierr)
      else
         global_count_dbl_tiny = 0
      endif
   else
      global_count_dbl_tiny = local_count
   endif

!-----------------------------------------------------------------------

 end function global_count_dbl_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_count_real = global_count_real_tiny(MASK, dist, field_loc)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
...
!***********************************************************************
 ...
 function global_count_real_tiny (MASK, dist, field_loc)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      global_count_real_tiny,    &! resulting global count
      local_count,         &! local count of subdomain
      ierr,                &! MPI error flag
      ib, ie, jb, je,      &! start,end of physical domain
      local_id,            &! local_id of root_block 
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_count, global_count_real_tiny, 1, &
                         MPI_INTEGER, MPI_SUM, dist%communicator, ierr)
      else
         global_count_real_tiny = 0
      endif
   else
      global_count_real_tiny = local_count
   endif

!-----------------------------------------------------------------------

 end function global_count_real_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_count_int = global_count_int_tiny(MASK, dist, field_loc)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
...
!***********************************************************************
 ...
 function global_count_int_tiny (MASK, dist, field_loc)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      global_count_int_tiny,    &! resulting global count
      local_count,         &! local count of subdomain
      ierr,                &! MPI error flag
      ib, ie, jb, je,      &! start,end of physical domain
      local_id,            &! local_id of root_block 
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_count, global_count_int_tiny, 1, &
                         MPI_INTEGER, MPI_SUM, dist%communicator, ierr)
      else
         global_count_int_tiny = 0
      endif
   else
      global_count_int_tiny = local_count
   endif

!-----------------------------------------------------------------------

 end function global_count_int_tiny
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      ...
      global_count_log = global_count_log_tiny(MASK, dist, field_loc)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
!EOC
...
!***********************************************************************
 ...
 function global_count_log_tiny (MASK, dist, field_loc)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      global_count_log_tiny     ! resulting global count
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_count, global_count_log_tiny, 1, &
                         MPI_INTEGER, MPI_SUM, dist%communicator, ierr)
      else
         global_count_log_tiny = 0
      endif
   else
      global_count_log_tiny = local_count
   endif

!-----------------------------------------------------------------------
!EOC

 end function global_count_log_tiny
