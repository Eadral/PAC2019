...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   interface update_matpow_halo  ! generic interface
     module procedure matpow_halo_2d_dbl
   end interface
...
!***********************************************************************
!BOP
! !IROUTINE: create_boundary_capcg
! !INTERFACE:

 subroutine create_boundary_capcg(newbndy, dist, &
                            ns_bndy_type, ew_bndy_type, &
                            nx_global, ny_global)
   ...
   block_loop2: do n=1,nblocks
      ...
      if (src_block_loc /= 0 .and. src_proc == my_task+1) then
         if (iblock_east == 0) then
            ...
            newbndy%local_ew_src_add(2,iloc_ew) = 0
            newbndy%local_ew_dst_block(iloc_ew) = src_block_loc
            newbndy%local_ew_dst_add(1,iloc_ew) = cacg_matlevel+block_size_x + 1
            newbndy%local_ew_dst_add(2,iloc_ew) = 1
         else if (iblock_west == 0) then
            iloc_ew = iloc_ew + 1
            newbndy%local_ew_src_block(iloc_ew) = 0
            ...
            newbndy%local_ew_dst_add(1,iloc_ew) = 1
            newbndy%local_ew_dst_add(2,iloc_ew) = 1
            !*** map to ie_dst_tiny for MIC tiny-block process
         else if (jblock_north == 0) then
            iloc_ns = iloc_ns + 1
            newbndy%local_ns_src_block(iloc_ns) = 0
            newbndy%local_ns_src_add(1,iloc_ns) = 0
            ...
            newbndy%local_ns_dst_block(iloc_ns) = src_block_loc
            newbndy%local_ns_dst_add(1,iloc_ns) = 1
            newbndy%local_ns_dst_add(2,iloc_ns) = cacg_matlevel+block_size_y + 1
         else if (jblock_south == 0) then
            iloc_ns = iloc_ns + 1
            newbndy%local_ns_src_block(iloc_ns) = 0
            newbndy%local_ns_src_add(1,iloc_ns) = 0
         ...
         if (dst_proc /= 0) then
            ...
            if (iblock_dst == iblock_east .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  iloc_ew = iloc_ew + 1
                  newbndy%local_ew_src_block(iloc_ew) = src_block_loc
                  newbndy%local_ew_src_add(1,iloc_ew) = cacg_matlevel+block_size_x - &
                                                        cacg_matlevel + 1
                  newbndy%local_ew_src_add(2,iloc_ew) = 1
                  ...
                  newbndy%ew_snd_proc (     imsg) = dst_proc
                  newbndy%ew_src_block(iblk,imsg) = src_block_loc
                  newbndy%ew_src_add(1,iblk,imsg) = cacg_matlevel+block_size_x - &
                                                    cacg_matlevel + 1
                  newbndy%ew_src_add(2,iblk,imsg) = 1
                  newbndy%nblocks_ew_snd(imsg) = &
                  newbndy%nblocks_ew_snd(imsg) + 1
            ...
            if (iblock_dst == iblock_west .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  iloc_ew = iloc_ew + 1
                  newbndy%local_ew_src_block(iloc_ew) = src_block_loc
                  newbndy%local_ew_src_add(1,iloc_ew) = cacg_matlevel + 1
                  newbndy%local_ew_src_add(2,iloc_ew) = 1
                  newbndy%local_ew_dst_block(iloc_ew) = dst_block_loc
                  newbndy%local_ew_dst_add(1,iloc_ew) = cacg_matlevel+block_size_x + 1
                  newbndy%local_ew_dst_add(2,iloc_ew) = 1
                  ...
                  newbndy%ew_snd_proc (     imsg) = dst_proc
                  newbndy%ew_src_block(iblk,imsg) = src_block_loc
                  newbndy%ew_src_add(1,iblk,imsg) = cacg_matlevel + 1
                  newbndy%ew_src_add(2,iblk,imsg) = 1
                  newbndy%nblocks_ew_snd(imsg) = &
                  newbndy%nblocks_ew_snd(imsg) + 1
               else if (dst_proc == my_task+1 .and. &
                  ...
                  newbndy%ew_rcv_proc (     imsg) = src_proc
                  newbndy%ew_dst_block(iblk,imsg) = dst_block_loc
                  newbndy%ew_dst_add(1,iblk,imsg) = cacg_matlevel+block_size_x + 1
                  newbndy%ew_dst_add(2,iblk,imsg) = 1
                  newbndy%nblocks_ew_rcv(imsg) = &
                  newbndy%nblocks_ew_rcv(imsg) + 1
               endif
            ...
            if ((iblock_dst == iblock_north .or. iblock_north < 0) .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  newbndy%local_ns_src_block(iloc_ns) = src_block_loc
                  newbndy%local_ns_src_add(1,iloc_ns) = 1
                  newbndy%local_ns_src_add(2,iloc_ns) = cacg_matlevel+block_size_y - &
                                                        cacg_matlevel + 1
                  newbndy%local_ns_dst_block(iloc_ns) = dst_block_loc
                  newbndy%local_ns_dst_add(1,iloc_ns) = 1
                  newbndy%local_ns_dst_add(2,iloc_ns) = 1
                  ...
                  if (iblock_north < 0) then !*** tripole boundary
                     ...
                     newbndy%local_ns_dst_block(iloc_ns) = -dst_block_loc
                     !*** copy cacg_matlevel+1 northern rows of physical
                     !*** domain into global north tripole buffer
                     newbndy%local_ns_src_add(1,iloc_ns) = &
                                        src_block%i_glob(cacg_matlevel+1)
                     newbndy%local_ns_src_add(2,iloc_ns) = &
                                        cacg_matlevel+block_size_y - cacg_matlevel
                     ...
                     !*** located on domain boundary
                     newbndy%local_ns_dst_add(1,iloc_ns) = &
                                          dst_block%i_glob(cacg_matlevel+1)
                     newbndy%local_ns_dst_add(2,iloc_ns) = & 
                                          cacg_matlevel+block_size_y
                  endif
               else if (src_proc == 0 .and. dst_proc == my_task+1) then
                  !*** source is land block so zero ghost cells
                  iloc_ns = iloc_ns + 1
                  ...
                  if (iblock_north < 0) then !*** tripole boundary
                     ...
                     newbndy%local_ns_dst_add(1,iloc_ns) = &
                                             dst_block%i_glob(cacg_matlevel+1)
                     newbndy%local_ns_dst_add(2,iloc_ns) = cacg_matlevel+block_size_y
                  endif
               else if (src_proc == my_task+1 .and. &
                        dst_proc /= my_task+1) then
                  !*** message must be sent
                  ...
                  newbndy%ns_src_block(iblk,imsg) = src_block_loc
                  newbndy%ns_src_add(1,iblk,imsg) = 1
                  newbndy%ns_src_add(2,iblk,imsg) = cacg_matlevel+block_size_y - &
                                                    cacg_matlevel + 1
                  newbndy%nblocks_ns_snd(imsg) = &
                  newbndy%nblocks_ns_snd(imsg) + 1
                  if (iblock_north < 0) then !*** tripole boundary
                     !*** need extra ghost cell for U points
                     newbndy%ns_src_add(2,iblk,imsg) = cacg_matlevel+block_size_y - cacg_matlevel
                  endif
               else if (dst_proc == my_task+1 .and. &
                        src_proc /= my_task+1) then
                  !*** message must be received
                  ...
                  if (iblock_north < 0) then !*** tripole
                     ...
                     newbndy%ns_dst_add(1,iblk,imsg) = &
                                            src_block%i_glob(cacg_matlevel+1)
                     !*** addresses for copying out of ghost buffer
                     newbndy%ns_dst_add(2,iblk,imsg) = cacg_matlevel+block_size_y
                     newbndy%ns_dst_add(3,iblk,imsg) = &
                                       dst_block%i_glob(cacg_matlevel+1)
                  endif
               endif
            ...
            if (iblock_dst == iblock_south .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  newbndy%local_ns_src_block(iloc_ns) = src_block_loc
                  newbndy%local_ns_src_add(1,iloc_ns) = 1
                  newbndy%local_ns_src_add(2,iloc_ns) = cacg_matlevel + 1
                  newbndy%local_ns_dst_block(iloc_ns) = dst_block_loc
                  newbndy%local_ns_dst_add(1,iloc_ns) = 1
                  newbndy%local_ns_dst_add(2,iloc_ns) = cacg_matlevel+block_size_y + 1
               else if (src_proc == 0 .and. dst_proc == my_task+1) then
                  ...
                  newbndy%ns_src_block(iblk,imsg) = src_block_loc
                  newbndy%ns_src_add(1,iblk,imsg) = 1
                  newbndy%ns_src_add(2,iblk,imsg) = cacg_matlevel+1
                  newbndy%nblocks_ns_snd(imsg) = &
                  newbndy%nblocks_ns_snd(imsg) + 1
               else if (dst_proc == my_task+1 .and. &
                        src_proc /= my_task+1) then
                  ...
                  newbndy%ns_dst_block(iblk,imsg) = dst_block_loc
                  newbndy%ns_dst_add(1,iblk,imsg) = 1
                  newbndy%ns_dst_add(2,iblk,imsg) = cacg_matlevel+block_size_y + 1
                  newbndy%nblocks_ns_rcv(imsg) = &
                  newbndy%nblocks_ns_rcv(imsg) + 1
               endif
            endif ! south neighbor
...
!-----------------------------------------------------------------------
   ...
   if (lalloc_tripole .and. .not. allocated(tripole_dbuf_capcg)) then
      allocate(tripole_dbuf_capcg(nx_global,cacg_matlevel+1), &
               tripole_dghost_capcg(nx_global,cacg_matlevel+1))
   endif

!-----------------------------------------------------------------------
!EOC

 end subroutine create_boundary_capcg


!***********************************************************************
!BOP
! !IROUTINE: update_matpow_halo
! !INTERFACE:

 subroutine matpow_halo_2d_dbl(ARRAY, in_bndy, grid_loc, field_type)
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ew_snd(cacg_matlevel, block_size_y, &
                       in_bndy%maxblocks_ew_snd, in_bndy%nmsg_ew_snd),&
            buf_ew_rcv(cacg_matlevel, block_size_y, &
                       in_bndy%maxblocks_ew_rcv, in_bndy%nmsg_ew_rcv))
   ...
   if (allocated(tripole_dbuf_capcg)) nx_global = size(tripole_dbuf_capcg,dim=1)
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_rcv
      ...
      bufsize = block_size_y*cacg_matlevel*in_bndy%nblocks_ew_rcv(n)
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_snd
      ...
      bufsize = block_size_y*cacg_matlevel*in_bndy%nblocks_ew_snd(n)

      do i=1,in_bndy%nblocks_ew_snd(n)
         ib_src    = in_bndy%ew_src_add(1,i,n)
         ie_src    = ib_src + cacg_matlevel - 1
         src_block = in_bndy%ew_src_block(i,n)
         buf_ew_snd(:,:,i,n) = ARRAY(ib_src:ie_src,cacg_matlevel+1:cacg_matlevel+block_size_y,src_block)
      end do
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nlocal_ew
      ...
      ie_src = ib_src + cacg_matlevel - 1
      ib_dst = in_bndy%local_ew_dst_add(1,n)
      ie_dst = ib_dst + cacg_matlevel - 1

      if (src_block /= 0) then
         ARRAY(ib_dst:ie_dst,cacg_matlevel+1:cacg_matlevel+block_size_y,dst_block) = &
         ARRAY(ib_src:ie_src,cacg_matlevel+1:cacg_matlevel+block_size_y,src_block)
      else
         ARRAY(ib_dst:ie_dst,:,dst_block) = c0
      endif
   end do
...
!-----------------------------------------------------------------------
   ...
   do k=1,in_bndy%nblocks_ew_rcv(n)
      ...
      ARRAY(ib_dst:ie_dst,cacg_matlevel+1:cacg_matlevel+block_size_y,dst_block) = buf_ew_rcv(:,:,k,n)
   end do
   end do
   !call timer_stop(bndy_2d_final)
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ns_snd((block_size_x+2*cacg_matlevel), cacg_matlevel+1, &
                       in_bndy%maxblocks_ns_snd, in_bndy%nmsg_ns_snd),&
            buf_ns_rcv((block_size_x+2*cacg_matlevel), cacg_matlevel+1, &
                       in_bndy%maxblocks_ns_rcv, in_bndy%nmsg_ns_rcv))
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ns_rcv
      ...
      bufsize = (block_size_x+2*cacg_matlevel)*(cacg_matlevel+1)*in_bndy%nblocks_ns_rcv(n)
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ns_snd
      ...
      bufsize = (block_size_x+2*cacg_matlevel)*(cacg_matlevel+1)*in_bndy%nblocks_ns_snd(n)

      do i=1,in_bndy%nblocks_ns_snd(n)
         jb_src    = in_bndy%ns_src_add(2,i,n)
         je_src    = jb_src + cacg_matlevel  ! cacg_matlevel+1 rows needed for tripole
         src_block = in_bndy%ns_src_block(i,n)
         buf_ns_snd(:,:,i,n) = ARRAY(:,jb_src:je_src,src_block)
      end do
...
!-----------------------------------------------------------------------
   ...
   if (allocated(tripole_dbuf_capcg)) tripole_dbuf_capcg = c0
   ...
   do n=1,in_bndy%nlocal_ns
      ...
      if (dst_block > 0) then ! straight local copy
         ...
         jb_src = in_bndy%local_ns_src_add(2,n)
         je_src = jb_src + cacg_matlevel - 1
         jb_dst = in_bndy%local_ns_dst_add(2,n)
         je_dst = jb_dst + cacg_matlevel - 1
         ...
         ! Debug sstep=8
         write(6,*) 'update_mat: has tripole:',my_task

         jb_src = in_bndy%local_ns_src_add(2,n)
         je_src = jb_src + cacg_matlevel ! need cacg_matlevel+1 rows for tripole
         ...
         ie_dst = ib_dst + (block_size_x) - 1
         if (ie_dst > nx_global) ie_dst = nx_global
         ib_src = cacg_matlevel + 1
         ie_src = ib_src + ie_dst - ib_dst
         if (src_block /= 0) then
            tripole_dbuf_capcg(ib_dst:ie_dst,:) = &
                  ARRAY(ib_src:ie_src,jb_src:je_src,src_block)
         endif
...
!-----------------------------------------------------------------------
   ...
   do k=1,in_bndy%nblocks_ns_rcv(n)
      ...
      if (dst_block > 0) then  ! normal receive
         ...
         ARRAY(:,jb_dst:je_dst,dst_block) = buf_ns_rcv(:,1:cacg_matlevel,k,n)
      else ! northern tripole bndy: copy into global tripole buffer
...
!-----------------------------------------------------------------------
   ...
   if (allocated(tripole_dbuf_capcg)) then
      ...
      case (field_loc_center)   ! cell center location
         ...
         !*** first row of ghost cell buffer is actually the last
         !*** row of physical domain (mostly for symmetry enforcement)
         tripole_dghost_capcg(:,1) = tripole_dbuf_capcg(:,cacg_matlevel+1)
      case (field_loc_NEcorner)   ! cell corner (velocity) location
         xoffset = 0
         yoffset = 0
         !*** enforce symmetry
         ...
         do i = 1, nx_global/2
            ib_dst = nx_global - i
            if (ib_dst == 0) ib_dst = nx_global
            xavg = p5*(abs(tripole_dbuf_capcg(i     ,cacg_matlevel+1)) + &
                       abs(tripole_dbuf_capcg(ib_dst,cacg_matlevel+1)))
            tripole_dghost_capcg(i     ,1) = sign(xavg, &
                                            tripole_dbuf_capcg(i,cacg_matlevel+1))
            tripole_dghost_capcg(ib_dst,1) = sign(xavg, &
                                            tripole_dbuf_capcg(ib_dst,cacg_matlevel+1))
         end do
         !*** catch nx_global point
         tripole_dghost_capcg(nx_global,1) = tripole_dbuf_capcg(nx_global,cacg_matlevel+1)
         tripole_dbuf_capcg(:,cacg_matlevel+1) = tripole_dghost_capcg(:,1)
      case (field_loc_Eface)   ! cell center location
         xoffset = 0
         yoffset = 1
         !*** first row of ghost cell buffer is actually the last
      ...
      !*** in the actual tripole buffer, the indices are:
      !*** nx_global + xoffset - i = ib_src - i
      !*** ny_global + yoffset - j - (ny_global - cacg_matlevel) + 1 =
      !***    cacg_matlevel + yoffset +1 - j = jb_src - j

      ib_src = nx_global + xoffset
      jb_src = cacg_matlevel + yoffset + 1

      do j=1,cacg_matlevel
      do i=1,nx_global
         tripole_dghost_capcg(i,1+j) = isign* &
                                 tripole_dbuf_capcg(ib_src-i, jb_src-j)
      end do
      end do
      ...
      do n=1,in_bndy%nlocal_ns
         ...
         if (dst_block < 0) then
            ...
            if (ib_src == 1) then  ! western boundary
               ...
               !*** then set up remaining indices to copy rest
               !*** of domain from tripole ghost cell buffer
               do i=1,cacg_matlevel
                  ARRAY(i,jb_dst:je_dst,dst_block) = &
                     tripole_dghost_capcg(nx_global-cacg_matlevel+i,:)
               end do
               ie_src = ib_src + block_size_x + cacg_matlevel - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = cacg_matlevel + 1
               ie_dst = ib_dst + (ie_src - ib_src)
            else
               ib_src = ib_src - cacg_matlevel
               ie_src = ib_src + (block_size_x+2*cacg_matlevel) - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = 1
               ie_dst = ib_dst + (ie_src - ib_src)
            endif
            ...
            ARRAY(ib_dst:ie_dst,jb_dst:je_dst,dst_block) = &
               tripole_dghost_capcg(ib_src:ie_src,:)
         endif
      ...
      do k=1,in_bndy%nblocks_ns_rcv(n)
         ...
         if (dst_block < 0) then
            ...
            jb_dst = in_bndy%ns_dst_add(2,k,n)
            je_dst = jb_dst + cacg_matlevel ! last phys row incl for symmetry
            ib_src = in_bndy%ns_dst_add(3,k,n)
            !*** ib_src is glob address of 1st point in physical
            !*** domain.  must now adjust to properly copy
            !*** east-west ghost cell info in the north boundary
...
!-----------------------------------------------------------------------
   ...
   deallocate(snd_request, rcv_request, snd_status, rcv_status)
      ...
      call exit_POP(sigAbort, 'NOT IMPL matpow_halo_2d_dbl_tiny')
   endif  ! lmic_proc
!-----------------------------------------------------------------------

 end subroutine matpow_halo_2d_dbl
