...
! !PUBLIC DATA MEMBERS:
   ...
   integer (int_kind), public :: &
      ...
      nblocks_clinic_mac   ! actual number of macro blocks on this processor
...
!----------------------------------------------------------------------
   ...
   call create_blocks_mac(nx_global, ny_global, trim(ew_boundary_type), &
                                            trim(ns_boundary_type))
   endif  ! lmic_proc
...
!----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      i,j,k,n,ierr              ,&! dummy loop indices
...
!----------------------------------------------------------------------
   ...
   do n=1,nblocks_tot
      ...
      if (nocn(n) > 0) nocn(n) = nx_block_mac*ny_block_mac
      endif  ! lmic_proc
   end do
...
!----------------------------------------------------------------------
!
!  allocate tiny blocks for the MIC process
!  assume clinic and tropic use the same distrb
!----------------------------------------------------------------------
   if (my_task == nproc_cpu_pn*nnode) then ! first MIC process
   write(6,*) 'distrb_tropic:nprocs',distrb_tropic%nprocs
   write(6,*) 'distrb_tropic:size_proc()',size(distrb_tropic%proc(:))
   do n=1,size(distrb_tropic%proc(:))
      write(6,*) 'distrb_tropic:i,proc,local_block', &
                  n,distrb_tropic%proc(n),distrb_tropic%local_block(n)
   end do
   write(6,*) 'distrb_tropic:end'
   endif
   if (lmic_proc) then
     nblocks_clinic_mac = size(blocks_clinic)
     call create_blocks_tiny(nx_global, ny_global, &
                       trim(ew_boundary_type), &
                       trim(ns_boundary_type), &
                       nblocks_clinic_mac, blocks_clinic)
     deallocate(blocks_clinic, blocks_tropic)
     call create_distribution_tiny(distrb_clinic)
     call create_distribution_tiny(distrb_tropic)
     call create_local_block_ids_tiny(blocks_clinic, distrb_clinic)
     call create_local_block_ids_tiny(blocks_tropic, distrb_tropic)
   endif  ! lmic_proc
...
!     call exit_POP(sigAbort,trim(outstring))
   ...
   distrb_clinic%local_block_ids => blocks_clinic(:)
   distrb_clinic%local_block_num = nblocks_clinic 
   distrb_tropic%local_block_ids => blocks_tropic(:)
   distrb_tropic%local_block_num = nblocks_tropic 
...
!   if (my_task == master_task) then
   ...
   write(6,*) 'distrb_tropic:end'
!   write(6,*) 'this_proc blocks:begin'
!   do n=1,distrb_clinic%local_block_num
!     this_block = get_block(distrb_clinic%local_block_ids(n),n)
!     write(6,*) this_block,this_block%i_glob(this_block%ib),this_block%j_glob(this_block%jb)
!   end do
!!   this_block = get_block(distrb_clinic%local_block_ids(1),1)
!!   this_block = get_block(this_block%block_id,1)
!!   write(6,*) this_block,this_block%i_glob(this_block%ib),this_block%j_glob(this_block%jb)
!   write(6,*) 'this_proc blocks:end'
   endif
   ! tuning debug
   !call exit_POP(sigAbort,'distrb_tropic completed.')
!----------------------------------------------------------------------
!
...
!----------------------------------------------------------------------
   ...
   call create_boundary(bndy_tropic, distrb_tropic, &
                        ...
                        trim(ew_boundary_type),     &
                        nx_global, ny_global)
   ! Added by Junmin, 2015.9.29
   ! for thicker boundary update in CA-PCG solver used by barotropic
   call create_boundary_capcg(bndy_capcg, distrb_tropic, &
                        trim(ns_boundary_type),     &
                        trim(ew_boundary_type),     &
                        nx_global, ny_global)
